...
│   └── requirements.txt
├── tests/
│   ├── smoke.test.mjs            # ✅ 50 فحص (الوضع المحلي)
│   ├── api.test.mjs              # ✅ 19 فحص (سيرفر وهمي)
│   └── e2e.test.mjs              # ✅ 14 فحص (ضد سيرفر بايثون حقيقي)
└── package.json
```
...
```bash
npm install
npm test           # 69 فحص: 50 محلي + 19 عبر سيرفر وهمي
...
npm run test:e2e   # 14 فحص ضد سيرفر بايثون حقيقي
```
...
- **لوحة إدارة الوظائف**: إضافة / تعديل / إغلاق / حذف وظيفة من الواجهة (تنعكس فوراً على الموقع)
